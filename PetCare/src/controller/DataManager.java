...
public class DataManager {
    ...
    // Cache dữ liệu (để tương thích với giao diện cũ dùng Vector)
    public static Vector<Booking> mockBookings = new Vector<>();
    public static Vector<Service> mockServices = new Vector<>();

    // Block static chạy khi khởi động app
    static {
        refreshCache();
    }

    // Hàm làm mới dữ liệu từ DB lên Cache
    ...
    // [MỚI] Cập nhật thông tin cá nhân (Gọi từ Tab Thông tin cá nhân)
    ...
    // Load danh sách khách hàng cho Admin
    ...
    public static void loadServicesToTable(DefaultTableModel model) {
        model.setRowCount(0);
        // Load từ cache (đã được refresh ở trên)
    ...
    public static void loadPetsToTable(DefaultTableModel model, String viewerUser, String role) {
        model.setRowCount(0);
        List<Pet> pets;
        if (role.equals("ADMIN")) pets = petRepo.getAllPets();
        else pets = petRepo.getPetsByOwner(viewerUser);
    ...
    public static void loadBookingsToTable(DefaultTableModel model, String viewerUser, String role) {
        ...
        if (role.equals("ADMIN")) bookings = bookingRepo.getAllBookings();
        else bookings = bookingRepo.getBookingsByCustomer(viewerUser);
    ...
    // [MỚI] Xác thực Email để reset pass
